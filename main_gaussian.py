...
from models_gaussian import KGReasoning
...
def parse_args(args=None):
    ...
    parser.add_argument('--geo', default='vec', type=str, choices=['vec', 'gaussian', 'beta'],
                        help='the reasoning model, vec for GQE, gaussian for PERM, beta for BetaE')
    ...
    parser.add_argument('-gaussianm', '--gaussian_mode', default="(none,0.02)", type=str,
                        help='(offset activation,center_reg) for PERM')
...
def load_data(args, tasks):
    ...
    valid_easy_answers = pickle.load(
        open(os.path.join(args.data_path, "valid-easy-answers.pkl"), 'rb'))  # todo: 没有数据啊，没法做高斯的
...
def main(args):
    ...
    for task in tasks:
        if 'n' in task and args.geo in ['gaussian', 'vec']:
    ...
    if args.geo in ['gaussian']:
        tmp_str = "g-{}-mode-{}".format(args.gamma, args.gaussian_mode)
    ...
    model = KGReasoning(
        ...
        geo=args.geo,
        use_cuda=args.cuda,
        gaussian_mode=eval_tuple(args.gaussian_mode),
    ...
    step = init_step
    if args.geo == 'gaussian':
        logging.info('gaussian mode = %s' % args.gaussian_mode)
    elif args.geo == 'beta':
